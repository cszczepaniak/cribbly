...
func (c Cell) MarshalBinary() ([]byte, error) {
	...
	// bsp handles values that can be nil pointers
	bsp := func(p string, s *string) string {
		if s == nil {
			return base64.StdEncoding.EncodeToString([]byte(p + "0"))
		}
		return base64.StdEncoding.EncodeToString([]byte(p + "1" +  *s))
	}
	...
	fmt.Fprintln(&b, bs("V"+c.Value), bsp("F", c.formula), bs("N"+c.NumFmt), c.date1904, c.Hidden, c.HMerge, c.VMerge, c.cellType, bs("HDS"+c.Hyperlink.DisplayString), bs("HL"+c.Hyperlink.Link), bs("HTT"+c.Hyperlink.Tooltip), c.num)
	return b.Bytes(), nil
}
...
func (c *Cell) UnmarshalBinary(data []byte) error {
	...
	// ubs unmarshals the base64  encoded cell data 
	ubs := func(p, s string) string {
		decoded, err := base64.StdEncoding.DecodeString(s)
		if err != nil {
			panic(err)
		}
		return strings.TrimPrefix(string(decoded), p)
	}

	// ubsp is like ubs, but it handles pointers that can be nil
	ubsp := func(p, s string) *string {
		decoded, err := base64.StdEncoding.DecodeString(s)
		if err != nil {
			panic(err)
		}
		unlabeled := strings.TrimPrefix(string(decoded), p)
		if unlabeled[0] == '0' {
			return nil
		}
		return sPtr(unlabeled[1:])
	}
	...
	c.Value = ubs("V", value)
	c.formula = ubsp("F", formula)
	c.NumFmt = ubs("N", numfmt)
	c.Hyperlink.DisplayString = ubs("HDS", hds)
	c.Hyperlink.Link = ubs("HL", hl)
	c.Hyperlink.Tooltip = ubs("HTT", htt)
	return err
}
...
func (c *Cell) Formula() string {
	if c.formula == nil {
		// This is sort of hacky, but it's better than breaking backwards compatibility 
		return ""
	}
	return *c.formula
}
