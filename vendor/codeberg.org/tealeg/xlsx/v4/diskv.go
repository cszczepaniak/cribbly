...
func (dvr *DiskVRow) readCell(key string) (*Cell, error) {
	...
	if c.formula, err = readStringPointer(buf); err != nil {
		return c, err
	}
...
func (dvr *DiskVRow) writeCell(c *Cell) error {
	...
	if err = writeStringPointer(&dvr.buf, c.formula); err != nil {
		return err
	}
...
func writeCell(buf *bytes.Buffer, c *Cell) error {
	...
	if err = writeStringPointer(buf, c.formula); err != nil {
		return err
	}
...
func readCell(reader *bytes.Reader) (*Cell, error) {
	...
	if c.formula, err = readStringPointer(reader); err != nil {
		return c, err
	}
